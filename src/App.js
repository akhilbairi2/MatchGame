...
class App extends Component {
  ...
  onThumbnailImg = id => {
    ...
    const updateList = imagesList.filter(each => each.id === id)
    const {imageUrl} = updateList[0]
    if (imgUrl === imageUrl) {
      const newImgUrl =
  ...
  render() {
    const {activeTab, time, score, imgUrl, isShow} = this.state
    const newImgList = imagesList.filter(each => each.category === activeTab)
    return (
        ...
        <div className="match-container">
          {isShow ? (
            <div className="math-card">
              <img src={imgUrl} alt="match" className="match-img" />
              <ul className="ul-container1">
                {tabsList.map(each => (
                  <TabItem
                    tabDetails={each}
                    key={each.tabId}
                    onTabButton={this.onTabButton}
              ...
              <ul className="thumbnail-container">
                {newImgList.map(each => (
                  <ThumbnailItem
                    ...
                    key={each.id}
                    onThumbnailImg={this.onThumbnailImg}
                  />
                ))}
              </ul>
            </div>
            ...
            <div className="result-container">
              ...
              <button type="button" className="try" onClick={this.onAgain}>
                <img
